...
<div class="category-fields"><label>项目名称 <span class="required-mark">*</span><input id="upload-project" name="project" list="project-names" maxlength="120" required placeholder="输入或选择项目名称"></label><label>平台分类 <span class="required-mark">*</span><select id="upload-platform" name="platform" required><option value="">请选择平台</option><?php foreach (FILE_PLATFORMS as $key => $label): ?><option value="<?=h($key)?>"><?=h($label)?></option><?php endforeach; ?></select></label><label>版本号 <span class="muted">（选填）</span><input id="upload-version" name="version" maxlength="60" placeholder="例如 v1.0.0，可不填"></label></div><datalist id="project-names"><?php foreach ($projects as $project): ?><option value="<?=h($project)?>"></option><?php endforeach; ?></datalist><p class="category-hint muted">项目为大分类，平台为小分类。本次选择的所有文件使用相同分类。</p>
...
<div class="category-browser"><label>项目<select id="project-filter" aria-label="项目大分类"><option value="all">全部项目</option><option value="unclassified">未分类</option><?php foreach ($projects as $project): ?><option value="<?=h('project:' . $project)?>"><?=h($project)?></option><?php endforeach; ?></select></label><span class="category-arrow">›</span><label>平台<select id="platform-filter" aria-label="平台小分类"><option value="all">全部平台</option><?php foreach (FILE_PLATFORMS as $key => $label): ?><option value="<?=h($key)?>"><?=h($label)?></option><?php endforeach; ?></select></label><p class="muted">按项目 → 平台筛选，版本号不必填写。</p></div>
<div class="table-scroll"><table><thead><tr><th>文件名</th><th>项目 / 平台</th><th>版本号</th><th>大小</th><th>上传时间</th><th>状态</th><th>操作</th></tr></thead><tbody id="file-rows">
...
?>
<tr class="file-row" data-name="<?=h($file['name'])?>" data-size="<?=h((string)$file['size'])?>" data-time="<?=h($file['time'])?>" data-type="<?=h($kind)?>" data-project="<?=h((string)($file['project'] ?? ''))?>" data-platform="<?=h((string)($file['platform'] ?? ''))?>"><td><div class="filename"><span class="file-icon <?=h($kind)?>"><?php icon('file'); ?></span><a href="<?=h($url)?>" title="<?=h($file['name'])?>"><?=h($file['name'])?></a></div></td><td><div class="category-cell"><strong><?=h($file['project'] ?? '未分类')?></strong><small><?=h(FILE_PLATFORMS[$file['platform'] ?? ''] ?? '未选择平台')?></small></div></td><td><?=h(($file['version'] ?? '') ?: '—')?></td><td><?=h(bytesLabel($file['size']))?></td><td><time datetime="<?=h($file['time'])?>"><?=h(str_replace(['T','Z'], [' ',' UTC'], $file['time']))?></time></td><td><span class="file-status">可分享</span></td><td><div class="row-actions"><input class="share-url" readonly value="<?=h($url)?>" aria-label="下载链接"><button class="copy text-button" type="button"><?php icon('link'); ?>复制链接</button><form method="post" class="delete"><?php token(); ?><input type="hidden" name="id" value="<?=h($id)?>"><button class="text-button danger-text" name="action" value="delete"><?php icon('trash'); ?>删除</button></form><details class="classify-details"><summary>编辑分类</summary><form method="post" class="classify-form"><?php token(); ?><input type="hidden" name="action" value="classify"><input type="hidden" name="id" value="<?=h($id)?>"><label>项目名称<input name="project" list="project-names" required maxlength="120" value="<?=h($file['project'] ?? '')?>"></label><label>平台<select name="platform" required><option value="">请选择平台</option><?php foreach (FILE_PLATFORMS as $key => $label): ?><option value="<?=h($key)?>"<?= ($file['platform'] ?? '') === $key ? ' selected' : '' ?>><?=h($label)?></option><?php endforeach; ?></select></label><label>版本号（选填）<input name="version" maxlength="60" value="<?=h($file['version'] ?? '')?>"></label><button class="primary" type="submit">保存分类</button></form></details></div></td></tr>
